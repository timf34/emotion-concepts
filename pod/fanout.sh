...
for m in $MODELS; do
  ...
  rp run "$name" --job phase1 --dotenv --env MODELS="$m" --env SHUTDOWN=stop -- "bash pod/fast_venv.sh && MODELS=$(python3 -c "import json;print({'gemma3_27b':'google/gemma-3-27b-it','gemma3_27b_pt':'google/gemma-3-27b-pt','gemma4_31b':'google/gemma-4-31B-it'}['$m'])") bash pod/predownload.sh && bash pod/run_phase1.sh"
