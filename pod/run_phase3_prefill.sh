...
$PY -m dprobe.cli prefill gemma4_31b --n $N --turn $TURN || { echo "!! gemma4 unsteered FAILED"; FAILED=1; }
...
[ -n "$CAL_CALM" ] && [ "$CAL_CALM" != "0.0" ] && { $PY -m dprobe.cli prefill gemma4_31b --n $N --turn $TURN --steer "calm:$CAL_CALM" || FAILED=1; }
[ -n "$CAL_AXIS" ] && [ "$CAL_AXIS" != "0.0" ] && { $PY -m dprobe.cli prefill gemma4_31b --n $N --turn $TURN --steer "assistant_axis:$CAL_AXIS" || FAILED=1; }
echo "================ gemma3_27b prefill (control)  $(date) ================"
MODELS=google/gemma-3-27b-it bash pod/predownload.sh
$PY -m dprobe.cli prefill gemma3_27b --n $N --turn $TURN || { echo "!! gemma3 control FAILED"; FAILED=1; }
