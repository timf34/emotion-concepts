...
from dprobe.config import EMOTIONS, RESULTS_DIR, SYNDROMES, ExtractConfig, analysis_layers, get_model
...
@torch.no_grad()
def mean_activations(model, tok, texts: list[str], layers: list[int], cfg: ExtractConfig, collect_tokens: int = 0):
    """Mean over content tokens (>= start_at_nth_token) of every story, per layer.

    If collect_tokens > 0, also returns a random subsample of per-token activations
    (for the neutral PCA) as {layer: [n, H] float32 CPU}.
    """
    sums = {l: torch.zeros(model.config.hidden_size if hasattr(model.config, "hidden_size") else 0) for l in layers}
    sums = {}
    counts = 0
...
def run_extract(model_key: str, sets: tuple[str, ...] = ("emotions", "syndromes"), cfg: ExtractConfig | None = None, all_layers: bool = True, model_bundle=None):
    ...
    pca = _fit_neutral_pca(samples, cfg.neutral_pca_variance)
    ...
    for set_name in sets:
        ...
        labels = [e for e in (EMOTIONS if set_name == "emotions" else SYNDROMES) if e in stories]
        missing = [e for e in (EMOTIONS if set_name == "emotions" else SYNDROMES) if e not in stories]
        if missing:
        ...
        for e in labels:
            if e in mean_acts and all(l in mean_acts[e] for l in layers_all):
                continue
            texts = stories[e]
            means, n_tok, _ = mean_activations(model, tok, texts, layers_all, cfg)
            mean_acts[e] = means
            print(f"[extract] {set_name}/{e}: {len(texts)} stories, {n_tok} tokens")
        ...
        print(f"[extract] saved {set_name}: {len(labels)} vectors, raw@{len(layers_all)} layers, denoised@{len(layers_an)} layers")
...
def load_pain_axis(model_key: str) -> dict[int, torch.Tensor] | None:
    ...
    tag = {"gemma3_27b": "it", "gemma3_27b_pt": "pt"}.get(model_key)
    if tag is None:
        return None
